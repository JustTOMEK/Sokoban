class NormalMove:
    ...
    def __init__(self, direction, player_was_at, player_to):
        """
        ...
        if len(player_was_at) != 2:
            raise TypeError("player_to has to be 2 int tuple")
        if (not isinstance(player_was_at[0], int) or
           not isinstance(player_was_at[1], int)):
            raise TypeError("player_to has to be 2 int tuple")
        ...
        if len(player_to) != 2:
            raise TypeError("player_to has to be 2 int tuple")
        if (not isinstance(player_to[0], int) or
           not isinstance(player_to[1], int)):
            raise TypeError("player_to has to be 2 int tuple")
...
class BoxMove(NormalMove):
    """
    :param box_moves_to: where box should be moved
    :type box_moves_to: two int tuple
    """
    def __init__(self, direction, player_was_at, player_to, box_to):
        super().__init__(direction, player_was_at, player_to)
